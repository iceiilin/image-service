...
function InventoryServiceFactory(
    ...
    function constructFileObject(link){
        var filename = fsOp.getFileNameFromStrSync(link);
        var fileStat = fsOp.getFileStatSync(link);
        return {
            'name': filename,
    ...
    InventoryService.prototype.initialize= function initialize() {
        ...
        fsOp.prepareWritableDirSync(this.isoDirPath);
        fsOp.prepareWritableDirSync(this.microkernelDirPath);
        fsOp.prepareWritableDirSync(this.httpFileServiceRootDir);

        this.inventoryFilePath = path.join(process.cwd(),
            configuration.get('inventoryFile', "./inventory.json"));
        db.load(this.inventoryFilePath);
    };
    ...
    InventoryService.prototype.validateImageObject = function validateImageObject(image) {
        ...
        var isEqual =  _.isEqual(imageKeys.sort(), _.keys(image).sort());
    ...
    InventoryService.prototype.setupDir = function setupDir(image) {
        ...
        return Promise.try(function(){
            if(!fsOp.checkPathReadableSync(isoFile)) {
    ...
    InventoryService.prototype.downloadIso = function downloadIso(image, link){
        if(validator.isURL(link)) {
            ...
            return fsOp.downloadIso(link, targetIsoPath)
                .then(function(){
                    if(fsOp.getFileSizeSync(targetIsoPath) === 0){
            ...
            return Promise.reject(new Errors.BadRequestError('Not a valid web link'));
        }
    };
            ...
            .then(function(images){
                ...
                return Promise.each(images, function(image){
                    ...
                    return fsOp.unmountIso(mountPath)
                        ...
                        .then(function(){
                            fsOp.removeDirAndEmptyParentSync(mountPath);
    ...
    InventoryService.prototype.getFiles = function getFiles(link) {
        ...
        _.forEach(filenameList, function (filename) {
            var filePath = path.join(link, filename);
            if(fs.existsSync(filePath)){ // exclude symbol links that has no real files associated
                value.push(constructFileObject(filePath));
            }
        });
